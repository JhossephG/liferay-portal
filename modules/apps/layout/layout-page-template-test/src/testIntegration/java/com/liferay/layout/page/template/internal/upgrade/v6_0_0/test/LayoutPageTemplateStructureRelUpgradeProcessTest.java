...
public class LayoutPageTemplateStructureRelUpgradeProcessTest
	...
	private String _getCollectionKey(
		...
		for (CollectionStyledLayoutStructureItem
			...
			JSONObject collectionJSONObject =
				collectionStyledLayoutStructureItems.getCollectionJSONObject();
	...
	private void _testUpgrade(String relationshipType) throws Exception {
		...
		Assert.assertEquals(
			key1,
			_getCollectionKey(_KEY_SUFFIX_1, _layout, segmentsExperienceId));
		Assert.assertEquals(
			key2,
			_getCollectionKey(
		...
		Assert.assertEquals(
			key2,
			_getCollectionKey(_KEY_SUFFIX_2, _layout, segmentsExperienceId));
