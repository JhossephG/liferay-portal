...
public class ObjectEntryInfoItemObjectProviderTest {
	...
	public void testGetInfoItemProxyObjectEntry() throws Exception {
		...
		_setUpProxyDTOObjectEntry(
			externalReferenceCode, dtoObjectEntry, objectEntry);

		ObjectEntry returnedObjectEntry = _assertGetInfoItem(
			new ERCInfoItemIdentifier(externalReferenceCode));

		Assert.assertEquals(userId, returnedObjectEntry.getUserId());
		Assert.assertEquals(userName, returnedObjectEntry.getUserName());
		Assert.assertEquals(createDate, returnedObjectEntry.getCreateDate());
		Assert.assertEquals(modifiedDate, returnedObjectEntry.getModifiedDate());
		Assert.assertEquals(
			defaultLanguageId, returnedObjectEntry.getDefaultLanguageId());
		Assert.assertEquals(
			externalReferenceCode,
			returnedObjectEntry.getExternalReferenceCode());
		Assert.assertEquals(
			objectEntryId, returnedObjectEntry.getObjectEntryId());
		Assert.assertEquals(scopeId, returnedObjectEntry.getGroupId());
