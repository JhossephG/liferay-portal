...
public class ObjectLayoutLocalServiceTest {
	...
	public void testAddObjectLayout() throws Exception {
			...
			_createObjectLayoutBox(
				_createObjectLayoutRow(
					_createObjectLayoutColumn(
						_customObjectDefinition.getObjectDefinitionId(), 10))));
		...
		AssertUtils.assertFailure(
			...
			() -> _addObjectLayout(
				_customObjectDefinition.getObjectDefinitionId(), true,
				objectLayoutTab,
				_createObjectLayoutTab(
					_createObjectLayoutBox(
						_createObjectLayoutRow(
							_createObjectLayoutColumn(
								_customObjectDefinition.getObjectDefinitionId(),
								objectField.getObjectFieldId(), 10))))));
		...
		AssertUtils.assertFailure(
			...
			() -> _addObjectLayout(
				_unmodifiableSystemObjectDefinition.getObjectDefinitionId(),
				false));
		AssertUtils.assertFailure(
			ObjectLayoutBoxTypeException.class,
			"Object layout box must have a type",
			() -> _addObjectLayout(
				_customObjectDefinition.getObjectDefinitionId(), false,
				objectLayoutTab,
				_createObjectLayoutTab(
		...
		_assertObjectLayout(
			false,
			_addObjectLayout(
				_modifiableSystemObjectDefinition.getObjectDefinitionId(), true,
				_createObjectLayoutTab(
					_createObjectLayoutBox(
						_createObjectLayoutRow(
							_createObjectLayoutColumn(
	...
	private ObjectLayout _addObjectLayout(
		...
		return _objectLayoutLocalService.addObjectLayout(
			...
			defaultObjectLayout, RandomTestUtil.randomLocaleStringMap(),
			...
			for (ObjectLayoutBox objectLayoutBox :
				...
				for (ObjectLayoutRow objectLayoutRow :
					...
					Assert.assertEquals(
						empty,
						ListUtil.isEmpty(
							_objectLayoutColumnPersistence.
	...
	private ObjectLayoutBox _createObjectLayoutBox(
		ObjectLayoutRow... objectLayoutRows) {

		return _createObjectLayoutBox(
			ObjectLayoutBoxConstants.TYPE_REGULAR, objectLayoutRows);
	}

	private ObjectLayoutBox _createObjectLayoutBox(
		String type, ObjectLayoutRow... objectLayoutRows) {
		...
		objectLayoutBox.setObjectLayoutRows(Arrays.asList(objectLayoutRows));

		return objectLayoutBox;
	}

	private ObjectLayoutColumn _createObjectLayoutColumn(
			long objectDefinitionId, int size)
		throws Exception {

		return _createObjectLayoutColumn(objectDefinitionId, 0, size);
	}

	private ObjectLayoutColumn _createObjectLayoutColumn(
			long objectDefinitionId, long objectFieldId, int size)
		throws Exception {
		...
		if (objectFieldId == 0) {
			ObjectField objectField = _objectFieldLocalService.fetchObjectField(
				objectDefinitionId, _OBJECT_FIELD_NAME);

			if (objectField == null) {
				objectField = _addCustomObjectField(
					_OBJECT_FIELD_NAME, objectDefinitionId);
			}
